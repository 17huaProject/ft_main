...
const afterWeek = new Date(bDaty.getTime() + parseInt(6 * 30 * 24 * 60 * 60 * 1000));
var _thisM = this;
const state = {
  ...
  commInfo: '',
  getParamString: '',
  serverHost: document.location.protocol +'//api.17hua.me', //正式,
...
const mutations = {
  ...
  saveWXconfig(state, conf) { //微信API调用配置
    ...
    var share = {
      ...
      sharelink: 'https://wx.17hua.me/#/list?u_id=' + (state.usrInfomation.user_id||17081707011),
  ...
  savePositionCode(state, obj) { //存取地点
    state.locationIN = obj;
  },
  ...
  saveGiftCardOrderInfo(state, info) { //礼品卡信息
    state.giftCardOrderInfo = info
  },
  ...
  saveInvoiceListInfo(state,obj){
    state.invoicelistInfo = obj
  },
  SaveStoreChangeExpress(state,statue){
    state.StoreChangeExpress = statue
  },
  SaveInvitedUserId(state,uid){
    state.invitedUserId = uid
  },
  SaveNewAvartarFromAPI(state,avater){
    state.usrInfomation.avatar = avater
  },
  // invoicelistInfo:{
  //   chargeArr:[],
  //   chusOrder:[]StoreChangeExpress
  // },
  ConectionPara(state, obj) { //请求参数补全
    let BaseArr = {
      ...
      ver: '1.0',
      client: 'WAP',
      test: document.location.protocol + '//' + window.location.host
    };
