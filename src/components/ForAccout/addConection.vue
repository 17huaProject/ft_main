<template>
    <div id="all_in_add">
        <div id="main_info_add">
            <div class="each_lin">
                姓名
                <input v-model="nam" maxlength="20">
            </div>
            <div class="each_lin">
                联系电话
            ...
            <div class="each_lin">
                电子邮箱
                <input v-model="eml" maxlength="50">
            </div>
            <div class="each_lin2">
                设为默认
...
export default {
    ...
    methods: {
        ...
        //     mat.phone = this.tel;
        //     mat.eml = this.eml;
        //     mat.seletOn = this.eosNumber % 2 === 0 ? false : true;
        //     let newArr = {
        //         thismat: mat,
        //         index: this.getParam,
        //     }
        //     typeof (this.$route.query.idx) === 'number' ? this.$store.commit('ReplaceConectArr', newArr) : this.$store.commit('PushConectArr', mat);
        //     this.$router.go(-1)
        // },
        addUserContact() {
            if (this.nam == '') {
            ...
            this.$http.post(this.$store.state.serverHost + '/yqhbsp/userinfo/addContact', postString).then(m => this.addUserContactCallBack(m.data)).catch(r => this.$toast.bottom('系统错误'));
        },
