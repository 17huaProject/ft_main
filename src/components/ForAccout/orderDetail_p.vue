    ...
    <div id="orderDetail">
        ...
        <div id="banTop">
            ...
            <div class="ac_info">
                ...
                <div class="ac_info_img" :style="{background: 'url(\''+this.imgTitle+'\')'+ 'center center no-repeat',backgroundSize:'cover'}"></div>
                <div class="ac_info_text">
                    <div>共计 {{orderinfo.number}} 个商品</div>
                    <div>总价：￥{{orderinfo.order_amount}}</div>
                    <div v-if="orderinfo.coupon_amount!=0&&orderinfo.order_status=='UNPAID'">代金券抵扣：￥{{orderinfo.coupon_amount}}</div>
                    <div v-if="orderinfo.balance_amount!=0&&orderinfo.order_status=='UNPAID'">画贝抵扣：￥{{orderinfo.balance_amount}}</div>
                </div>
            </div>
            <div class="ac_ad_info" v-if="orderinfo.address">
         ...
         <div id="banBottom" v-if="orderinfo.order_status=='SUCCESS'">
            <div class="jinTds">
        ...
        <div class="frieWork_e" v-show="orderinfo.order_status=='UNPAID'">
            <div class="co_moution_box_div">应付款：</div>
...
export default {
  ...
  methods: {
    ...
    getOrderInfo(orderid) {
      ...
      this.$http
        .get(this.$store.state.serverHost + "/yqhbsp/order/show" + getString)
        .then(m => this.setOrderInfo(m.data))
    ...
    rePaytheOrder() {
      let comInfo = {
        ...
        OrderInfo: {
          name: this.orderinfo.order_name,
    ...
    setOrderInfo(data) {
      if (data.errcode === 200) {
        ...
        this.imgTitle =
          data.record.order_img.indexOf("http") < 0
            ? this.$store.state.SyetemBaseConfig.image_prefix +
              data.record.order_img
            : "" + data.record.order_img.replace(/\http:/g, "");
        ...
        try {
          this.qrCode =
            data.record.tickets[0].ticket_code.replace(/(.{4})/g, "$1\n") || 0;
        } catch (error) {
          console.log(error);
        }
      } else {
    ...
    ComputetTime(data) {
      let st = data.sys_time.replace(/\-/g, "/"),
        ct = this.orderinfo.create_time.replace(/\-/g, "/");
    ...
    jumpToamap() {
      ...
      this.$router.push({
        name: "amapif",
        query: { lginfo: pt, lgname: this.orderinfo.address }
      });
    },
    jumpToACD(acNum) {
