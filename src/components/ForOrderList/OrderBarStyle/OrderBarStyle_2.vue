    ...
    <div class="orderpills">
        ...
        <div class="mainOrderinfo" @click="routertoDetail()">
            <div class="leftImg" :style="{background: 'url(\''+ this.getImgAddr(eachinfo.order_img)+'\')'+ 'center center no-repeat',backgroundSize:'auto 100%'}">
                ...
                <!-- <div v-if="eachinfo.order_status=='SUCCESS'" style="background-color:#ff7d54">可使用</div>
                <div v-else-if="eachinfo.order_status=='UNPAID'" style="background-color:#FDA649">未支付</div>
                <div v-else-if="eachinfo.order_status=='PENDING'" style="background-color:#C29B70">已使用</div>
            ...
            <div class="rightInfo">
                <div>商品名称：{{eachinfo.order_name}}</div>
                <div style="font-size:12px">数量：{{eachinfo.number}}</div>
                <div>总价：￥{{eachinfo.order_amount}}</div>
            </div>
            <div class="OrderStatueText" v-if="eachinfo.order_status=='SUCCESS'" >支付成功</div>
            <div class="OrderStatueText" v-else-if="eachinfo.order_status=='CLOSED'">已关闭</div>
        </div>
        <div class="endButton" v-if="eachinfo.order_status=='UNPAID'">
            <div @click="routertoDetail()">去支付</div>
        </div>
    </div>
</template>
<script>
...
export default {
    methods: {
        ...
        routertoDetail() {
            this.$router.push({ name: 'ordtp', query: { orderID: this.eachinfo.order_id } })
        },
