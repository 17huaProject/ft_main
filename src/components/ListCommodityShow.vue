<template>
    <div class="ListMain_c">
        <div class="top_fun_ara" style="z-index:4;margin-top:">
            <div class="top_func">
                <input placeholder="请输入商品进行搜索" v-model="SearchBar" @keyup.13="saveKeyword">
                <img class="search_img" src="../../static/obj_img/home_icon_search.png" @click="saveKeyword">
            </div>
        </div>
        <div class="ListArea_inbox_Com">
        ...
        <div id="listBottom" style="border-top:1px soild #eee">
            ...
            <div v-show="AtTop">
                <img src="../../static/obj_img/tab_shop_selected_icon.png">
                <div style="color:#ff3f5c">商城</div>
            </div>
...
export default {
    ...
    methods: {
        changeUpOrDown(state) {
            return state ? 'top_fun_ara animated fadeInDown' : 'top_fun_ara animated fadeOutUp'
        },
        getListProduct() {
            ...
            this.$http.get(this.$store.state.serverHost + '/yqhbsp/goods/index' + getString).then(m => this.setPaberList_a(m.data)).catch(r => console.log(r));
        },
        getTop() {
            window.scrollTo(0, 0)
        },
        ...
        setPaberList_a(data) {
            if (data.errcode === 200) {
                data.records.length >=5 ? this.pageRoll = true : '';
    ...
    watch: {
        ...
        ListConfig: {
            handler: function(val, oldVal) {
                ...
                this.getListProduct(val.page_no == 1)
            },
            deep: true
        },
        rollScrollCan: {
